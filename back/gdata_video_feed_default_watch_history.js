...
class FeedsApiVideos {
    ...
    static async handleRecommendationsRequest(req, res, accessToken) {
        ...
        try {
            ...
            const logFilePath = path.join(logsDir, `history-browse-response-${timestamp}.json`);
    ...
    static async getVideos(req, res) {
        ...
        videoData2 = await FeedsApiVideos.handleSubscriptionsRequest(req, res, accessToken);

        let combinedVideos = [...videoData, ...videoData2];

        FeedsApiVideos.shuffleArray(combinedVideos);
...
router.get('/feeds/api/users/default/watch_history', FeedsApiVideos.getVideos);
