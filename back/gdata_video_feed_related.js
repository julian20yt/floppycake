...
class FeedsApiVideos {
    ...
    static async handleNextRequest(req, res, videoId) {
        ...
        const postData = {
          context: {
              client: {
                  ...
                  webpSupport: false,
                  configInfo: {
                      appInstallData: "COnysL0GEKDWzhwQvNHOHBDmz7EFEIPDsQUQwrfOHBCHw7EFEMHNsQUQuJnOHBDPuc4cEMn3rwUQwLfOHBCB1rEFEIHNzhwQiLDOHBC9mbAFEO25sQUQvoqwBRCPw7EFEPyyzhwQndCwBRCZmLEFEOSVsAUQ0ZTOHBCL1LEFELK-zhwQy5rOHBCOwc4cEMzfrgUQktnOHBC14_8SEOvo_hIQ0-GvBRCUu84cEIiHsAUQh6zOHBCO0LEFEKLUsQUQmdL_EhDCyc4cEJmNsQUQjtexBRCFp7EFEJT-sAUQ5MnOHBDlubEFEJrOsQUQiqGxBRDeyM4cEN-0zhwQxLvOHBDerbEFENCNsAUQiOOvBRDnms4cEOrDrwUQr8LOHBCT_K8FEIS9zhwQt-r-EhDevM4cEMjYsQUQjdSxBRC9tq4FEJK4zhwQrsHOHBD-5f8SEPirsQUQ2NXOHBDE2LEFEMnmsAUQvLLOHBDB2v8SEJLLsQUQxtixBRD2q7AFEIHDsQUQ-rjOHBDK2LEFEOfQzhwQjNCxBRDWvs4cKiBDQU1TRWhVSi1acS1EUHJpRVlISjJ3dm1vUWdkQnc9PQ%3D%3D"
                  },
                  tvAppInfo: {
                  ...
                  deviceExperimentId: "ChxOelEyT0RVd056WTFOREV4TURJMk9EZzJNQT09EOnysL0GGKTwlb0G",
                  rolloutToken: "CJfLqI7Ivb_y1QEQnNWkuauwiwMYrqmnwfy6iwM%3D"
              },
              user: {
        ...
        try {
            ...
            const logFilePath = path.join(logsDir, `next-realated-response-${timestamp}.json`);
            ...
            const logFilePathIni = path.join(logsDir, `next-intermediate-response-${timestamp}.json`);
    ...
    static async convertToIntermediateForm(responseData) {
        ...
        const items = responseData.contents.singleColumnWatchNextResults.pivot.sectionListRenderer.contents;

        if (Array.isArray(items)) {
            // Collect all the promises for video processing
            const allPromises = items.map(async item => {
                const video = item?.shelfRenderer?.content?.horizontalListRenderer?.items?.[0].tileRenderer;

                if (video) {
                    console.log("yap dollar", JSON.stringify(video, null, 2));
                    ...
                    const pfpUrl = profileData.pfpUrl;

                    console.log("pfp url testy: " + JSON.stringify({ pfpUrl }, null, 2));

                          
                    const descriptionText = profileData.description;

                    const browseId = profileData.browseId;
                    ...
                    const videoData = {
                        ...
                        pfp: pfpUrl || "https://yt3.ggpht.com/ytc/AIdro_mrBFeElQkp-3jLyFGRPGjkMkgY2ZC8D7IoaQGp0-U=s48-c-k-c0x00ffffff-no-rj",
    ...
    static async getVideos(req, res) {
        ...
        if (!videoId) {
            return res.status(418).json({ error: "Missing videoId in request." });
        }
        
        let videoData;

        videoData = await FeedsApiVideos.handleNextRequest(req, res, videoId);
...
router.get('/feeds/api/videos/:videoId/related', FeedsApiVideos.getVideos);
